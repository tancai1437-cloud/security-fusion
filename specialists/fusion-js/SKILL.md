...
5. 实现阶段先把最小可运行程序落盘，再用实际运行逐项验证和修正，每步解决一个具体问题。要求独立复现时，把交付文件放到没有原样本的目录，实际运行新输入并与原实现逐项比较，至少覆盖多字段对象、数组顺序、Unicode、缺省/null 和数字序列化中适用的边界。先修不一致项，再交付；单个样例一致不能证明复现成功。缺环境条件则返回缺口。
