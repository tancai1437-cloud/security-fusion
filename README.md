...
[DSH 路由执行](references/host-adapter.md#先路由再实际调用)现在先返回当前专项方法和真实工具 schema，再由 execute 调用；工具缺失、多候选、自定义替代分别处理。路由按会话与方法/schema 版本绑定，真实回执记录具体路由。只加载不执行也纳入有限纠正；纯文本 Skill 仍没有宿主拦截能力。

本轮 [路由实测记录](references/routing-validation-2026-09-26.md)保留模型超时、参数错误和暂停绕过等失败样本，以及对应修正；不把加载 Skill 或进程退出码 0 当作任务完成。

[基础能力验证](references/foundations-validation-2026-09-26.md)：150 项本地测试，147 通过、3 跳过；真实 DSH 两轮任务在压缩后复用原证据完成限定交付，全程一次目标请求。保留前两轮失败与修复记录，不将此推断为所有复杂任务均已可靠。
