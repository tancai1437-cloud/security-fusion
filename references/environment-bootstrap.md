...
此流程由当前 Agent 在用户要求安装/初始化，或当前检查确实缺少可用 MCP 能力时执行。开始实际任务不意味着必须先初始化全部环境：已有本地命令可直接 start/run 取得业务证据，不要求为其生成 MCP 收据；所需 MCP 已有有效绑定则直接复用。只补齐当前检查所需能力，其他工具不作为执行前置条件。明确的初始化请求仍完成检测、补齐和验收，不能只交待办。只处理目标运行机器，不在协助发布 Skill 的开发机安装安全软件。保留主路由 → 专项 → 执行三层结构。
