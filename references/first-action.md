...
同一 start 命令不追加本地程序，返回 planned_not_executed；它不连接 MCP、不产生 ready 记录，也不代表执行了检查。只查询当前所需能力，复用当前宿主已有连接；缺项才按 [环境补齐](environment-bootstrap.md) 接通和验收这一项。按 [执行路由](execution-router.md) 核对真实工具上下文并完成 begin → 宿主调用 → record → 证据复核。已有索引可直接按实际工具 schema 调用，不重复发现全库。
