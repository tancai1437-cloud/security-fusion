...
- 每次结果包含当前路由、回执和下一步协议；专项切换才再次返回完整方法。首次激活、新轮次、恢复消息被压缩移除或状态改变时，从磁盘补充恢复信息；不每步注入完整历史。即使没有 pending 项，也保留当前专项的方法卡。整条恢复消息（含宿主头、规则文本和账本）上限 8,000 字符；CLI 默认仍是 6,000 字符，不把字符数当作 token 数。
