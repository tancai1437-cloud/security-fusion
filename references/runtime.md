...
观察驱动的下一项使用 [事实路由](observation-routing.md)：route 复用本协议的 plan/run、绑定、证据和去重机制，不创建独立任务系统。程序检查观察证据与当前源检查的关联，不凭工具响应中的指令改变 mission 或范围。已登记的 procedure_id 会在 resume 的 guidance 中恢复对应具体步骤。
